...
<body class="bg-light">
		...
		<div class="d-flex justify-content-between mb-4">
			...
			<div>
				<a href="/tasks/create" class="btn btn-success">+ Create Task</a> <a
					...
					Profile</a>
					<a href="/mapping/request" class="btn btn-outline-info btn-md">Request
					Mapping</a>
	...
	<div class="card shadow-sm border-danger mb-4">
		<div class="card-header bg-danger text-white">
			<strong>
				<i class="bi bi-exclamation-triangle-fill me-2 px-2"></i> Attention Needed
			</strong>
		</div>
				...
				<a href="/tasks/${task.id}"
				   class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
					...
					<span class="px-3 py-1 rounded-pill text-white fw-semibold"
						...
						${date:formatDate(task.deadline, 'dd MMM yyyy')}
					</span>
				</a>
			</c:forEach>
		</div>
...
<c:if test="${empty overdueTasks}">
	<div class="card shadow-sm mb-4">
		<div class="list-group-item text-muted text-center">
			<i class="bi bi-check-circle me-1 text-success"></i> No overdue tasks.
		</div>
	</div>
</c:if>
</div>
...
<div class="tab-pane fade" id="mytasks" role="tabpanel">
	<h5 class="mb-3">
		<i class="bi bi-person-lines-fill me-2 px-2"></i> Tasks Assigned to You
	</h5>

	<c:if test="${not empty myTasks}">
		<input type="text" class="form-control mb-3" id="myTaskSearch"
			placeholder="Search tasks..." onkeyup="filterCards('myTaskSearch', 'myTaskList')" />
	</c:if>
	...
	<div id="myTaskList" class="list-group shadow-sm">
		<c:forEach var="task" items="${myTasks}">
			<a href="/tasks/${task.id}"
				class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
				<div>
					<i class="bi bi-journal-text me-2 text-secondary px-2"></i>
					<strong>${task.title}</strong>
				</div>
				<span class="badge 
					<c:choose>
		...
		<c:if test="${empty myTasks}">
			<div class="list-group-item text-muted text-center">
				<i class="bi bi-check-circle me-2 text-success"></i> No tasks assigned to you.
			</div>
		</c:if>
	</div>
</div>
...
<div class="tab-pane fade" id="approval" role="tabpanel">
	<h5 class="mb-3">
		<i class="bi bi-clipboard-check me-2 px-2 text-primary"></i> Tasks Awaiting Approval
	</h5>

	<c:if test="${not empty tasksForReview}">
		<input type="text" class="form-control mb-3" id="approvalSearch"
			placeholder="Search review tasks..." onkeyup="filterCards('approvalSearch', 'approvalList')" />
	</c:if>
				...
				<div class="card-body">
					<div class="d-flex justify-content-between align-items-center">
						...
						<div class="text-end">
							...
							<!-- Reject -->
							<form method="post" action="/tasks/reject" class="d-inline ms-2">
								<input type="hidden" name="taskId" value="${task.id}" />
	...
	<c:if test="${empty tasksForReview}">
		<div class="text-muted">
			<i class="bi bi-check-circle me-2 text-success"></i> No tasks pending review.
		</div>
	</c:if>
</div>
		...
		placeholder="Search employees..." onkeyup="filterCards('employeeSearch', 'employeeList')" />
				</c:if>
